...
def test_paper_r1_truth_matches_msprime_likelihood():
    data = load_snp_dataset(REP0)
    simple = tskit.load(REP0 / "r1.trees")
    reference = msprime.log_mutation_likelihood(simple, mutation_rate=1e-8)
    for filename in ("r1.trees", "r1.full.trees"):
        result = evaluate_infinite_sites(tskit.load(REP0 / filename), data, mutation_rate=1e-8)
        assert result.log_likelihood == pytest.approx(reference, rel=0, abs=1e-9)
