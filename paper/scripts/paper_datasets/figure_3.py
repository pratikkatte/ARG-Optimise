...
from paper.scripts.paper_datasets.evaluate import load_draws, resolve, require, sha256, write_json
from paper.scripts.plot_paper_posterior_agreement import compare_clades
...
def main(argv=None):
    ...
    write_json(output/'provenance.json',dict(options=options,config_sha256=sha256(args.config),
        ...
        source_sha256={str(p.relative_to(ROOT)):sha256(p) for p in (Path(__file__),
            Path(__file__).with_name('evaluate.py'),ROOT/'paper/scripts/plot_paper_posterior_agreement.py',
