...
from paper.poc.poc_reference import exact_evidence, rejection_sample, sample_prior, trajectory
...
def source_hashes():
    return {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
            for folder in ('env','policy','gfn','training','validation','paper/poc') for p in sorted((ROOT/folder).rglob('*.py'))} | {
...
def main():
    ...
    p.add_argument('--config',type=Path,default=ROOT/'paper/poc/config.yaml')
