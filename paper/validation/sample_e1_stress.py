...
def main():
    ...
    out=Path(cfg['output']).parent/'stress'; out.mkdir(parents=True,exist_ok=True)
    ...
    for di,job in enumerate(cfg['datasets']):
        ...
        observed=load_snp_dataset(PROJECT/'validation/datasets/paper_datasets'/job['dataset']/'rep0')
