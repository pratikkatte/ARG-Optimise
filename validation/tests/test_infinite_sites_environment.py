...
@pytest.mark.skipif(not REP0.exists(),reason='local rep0 unavailable')
def test_rep0_full_ancestry_replay_and_cache_reconstruction():
    ...
    state,snapshots = truth_replay(env,tskit.load(REP0/'sim_5k_mr20.full.trees'))
    assert state.is_done and len(state.actions)==25
    assert state.partial_log_likelihood == pytest.approx(-733.4768140242744,rel=0,abs=1e-9)
    assert_independent(env,state)
...
def test_rep0_generated_candidates(seed):
    env = SimpleARGEnvironment(snp_data=load_snp_dataset(REP0), population_size=100000,
                              mutation_rate=2.5e-8,recombination_rate=1.25e-9,seed=seed)
...
@pytest.mark.skipif(not REP0.exists(),reason='local rep0 unavailable')
def test_diagnostic_cli_without_ground_truth(tmp_path):
    import json
    import shutil
    import subprocess
    import sys
    bundle=tmp_path/'observations'; bundle.mkdir()
    meta=json.loads((REP0/'metadata.json').read_text())
    for filename in ('metadata.json',meta['files']['vcf'],meta['files']['position_map']):
        shutil.copyfile(REP0/filename,bundle/filename)
    before={p.name:p.read_bytes() for p in bundle.iterdir()}
    script=Path(__file__).resolve().parents[1]/'scripts/sample_infinite_sites.py'
    output=tmp_path/'diagnostic'
    result=subprocess.run([sys.executable,str(script),'--replicate-dir',str(bundle),
                           '--seed','8','--output-dir',str(output)],capture_output=True,text=True,check=True)
    report=json.loads(result.stdout)
    assert report['posterior_samples'] is False
    assert report==json.loads((output/'manifest.json').read_text())
    assert before=={p.name:p.read_bytes() for p in bundle.iterdir()}
    ts=tskit.load(output/'arg_seed8.trees')
    assert ts.num_sites==0 and ts.num_mutations==0
    independent=evaluate_infinite_sites(ts,load_snp_dataset(bundle),mutation_rate=2.5e-8)
    assert report['results'][0]['log_likelihood']==pytest.approx(independent.log_likelihood,abs=1e-9)
