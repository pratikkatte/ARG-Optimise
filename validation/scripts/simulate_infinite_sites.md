...
```sh
python3 validation/scripts/simulate_infinite_sites.py --config validation/config/config.yaml
```

The script also uses that YAML file by default. It is independent of
...
Edit the new `config.yaml` to set rates, sample size, length, replicates, seeds,
...
```sh
python3 -m pytest validation/tests/test_simulate_infinite_sites.py -q
```
