...
conda run --no-capture-output -n phylogfn_orig python \
  validation/scripts/paper_datasets/evaluate.py

# 2. Figure 2: posterior-mean pairwise TMRCA versus simulated truth
conda run --no-capture-output -n phylogfn_orig python \
  validation/scripts/paper_datasets/figure_2.py

# 3. Figure 3: posterior-summary agreement with ARGInfer and SINGER
conda run --no-capture-output -n phylogfn_orig python \
  validation/scripts/paper_datasets/figure_3.py
```
...
```bash
python validation/scripts/paper_datasets/evaluate.py --config /path/to/config.yaml
```
...
```bash
python -m pytest -q validation/scripts/paper_datasets/test_evaluate.py
```
...
```bash
python -m pytest -q validation/scripts/paper_datasets/test_figure_2.py
```
