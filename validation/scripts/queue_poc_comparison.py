...
def main():
    ...
    if process is not None:
        ...
        while process.is_running() and process.status()!=psutil.STATUS_ZOMBIE:
            time.sleep(15)
    config=yaml.safe_load((ROOT/'validation/config/poc.yaml').read_text())
    for seed in config['training']['seeds']:
        ...
        if not path.exists() or json.loads(path.read_text())['step']!=config['training']['steps']:
            ...
            return 1
    command=[sys.executable,'-u','validation/scripts/run_poc.py','--config','validation/config/poc.yaml',
